...
class CrossoverOperator:
    ...
    def SinlgePointCrossover(self, chromosome1, chromosome2, gene_length):
        L1 = len(chromosome1)
        gene_length = gene_length
        n1 = L1 / gene_length

        crossover_point = 1+np.random.randint(n1)
        j = np.copy(crossover_point)

        crossed1 = np.append(chromosome1[:crossover_point] chromosome2[-crossover_point+1:])
        crossed2 = np.append(chromosome2[:crossover_point] chromosome1[-crossover_point+1:])
    ...
    def TwoPointCrossover(self, chromosome1, chromosome2, gene_length):
        ...
        cut_end1 = np.random.randint(cut_start1, n1)
        cut_start2 = np.random.randint(n2)
        cut_end2 = np.random.randint(cut_start2, n2)
        ...
        crossed1 = np.append(np.append(chromosome1[0:cut_start1*gene_length], piece2), chromosome1[-cut_start1*gene_length+1:])
        crossed2 = np.append(np.append(chromosome2[0:cut_start2*gene_length], piece1), chromosome2[-cut_start2*gene_length+1:])
